...
class FSKMod {
    ...
    resumeAudio() {
        ...
        webAudioTouchUnlock(this.audioContext) .then(function (unlocked) {
            if(unlocked) {
            } else {
            }
          }, function(reason) {
    ...
    isMacSafari() { 
       console.log('ismac:' + /macintosh|mac os x/i.test(navigator.userAgent) );
       console.log('issafari:' + (navigator.userAgent.indexOf('Safari') >= 0) );
    ...
    async generate(bitStreamArray) {
        ...
        console.log('encode data len:' + data.length);
